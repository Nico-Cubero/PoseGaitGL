...
class BaseModel(MetaModel, nn.Module):
    ...
    def __init__(self, cfgs, training, out_dir='output/'):
        ...
        out_dir:
            Directory where output files will be stored. Defaults to 'output/'.
        """
        ...
        self.save_path = osp.join(out_dir, cfgs['data_cfg']['dataset_name'],
    ...
    def _load_ckpt(self, save_name):
        ...
        checkpoint = torch.load(save_name, map_location=torch.device(
            "cuda", self.device), weights_only=True)
    ...
    def run_train(model):
        ...
        for inputs in model.train_loader:
            ...
            with autocast(device_type='cuda', enabled=model.engine_cfg['enable_float16']):
                retval = model(ipts)
