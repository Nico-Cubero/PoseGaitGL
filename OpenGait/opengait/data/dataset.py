...
class DataSet(tordata.Dataset):
    def __init__(self, data_cfg, training):
        ...
        self.train_set_sel_types = None # By default all seq. types are used
        self.sel_channel_idxs = data_cfg.get('sel_channel_idx', None)
        # Prevent keeping channel dim when single dim is selected
        if isinstance(self.sel_channel_idxs, (list, tuple)) and len(self.sel_channel_idxs) == 1:
            self.sel_channel_idxs = self.sel_channel_idxs[0]
    ...
    def __loader__(self, paths):
        ...
        for pth in paths:
            ...
            # Filter selected channels (if sel_channel_idx is specified)
            if self.sel_channel_idxs:
                _ = self.__filter_channels(_)
            data_list.append(_)
    ...
    def __dataset_parser(self, data_config, training):
        ...
        if training and "TRAIN_SET_TYPES" in partition:
            self.train_set_sel_types = partition["TRAIN_SET_TYPES"]
        else:
            self.train_set_sel_types = None
        ...
        def get_seqs_info_list(label_set):
            ...
            for lab in label_set:
                for typ in sorted(os.listdir(osp.join(dataset_root, lab))):
                    if self.train_set_sel_types and typ not in self.train_set_sel_types:
                        # Discard non-selected video types (only for training)
                        continue
    ...
    def __filter_channels(self, data):
        return data[:, self.sel_channel_idxs]
