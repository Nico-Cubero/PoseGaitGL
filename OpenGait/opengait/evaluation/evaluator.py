...
from .metric import cuda_dist, compute_ACC_mAP#mean_iou, cuda_dist, compute_ACC_mAP, evaluate_rank, evaluate_many
...
def evaluation_tum_gaid(data, dataset, metric='euc', num_rank=1):
    msg_mgr = get_msg_mgr()
    feature, label, seq_type = data['embeddings'], data['labels'], data['types']
    ...
    # Non-elapsed time seq. conf
    probe_seq_dict = {'TUM_GAID': [['n05', 'n06'], ['b01', 'b02'], ['s01', 's02']]}
    gallery_seq_dict = {'TUM_GAID': [['n01', 'n02', 'n03', 'n04']]}

    # Elapsed time seq. conf
    probe_seq_dict_T = {'TUM_GAID': [['n11', 'n12'], ['b03', 'b04'], ['s03', 's04']]}
    gallery_seq_dict_T = {'TUM_GAID': [['n07', 'n08', 'n09', 'n010']]}

    if dataset not in (probe_seq_dict or gallery_seq_dict):
        raise KeyError("DataSet %s hasn't been supported !" % dataset)

    # Separate elapsed-time subjects from normal (non-elapsed) subjects
    t_sub_search = np.isin(seq_type, 'n07')
    t_sub_labels = np.unique(label[t_sub_search])
    t_sub_mask = np.isin(label, t_sub_labels)
    nm_sub_labels = np.unique(label[~t_sub_mask])
    
    t_sub_num = len(t_sub_labels)
    nm_sub_num = len(nm_sub_labels)
    total_sub_num = t_sub_num + nm_sub_num
    assert total_sub_num == 155, 'Total number of subjects is not 155 but ' + str(total_sub_num)
    msg_mgr.log_info('Subject description:\n' \
                    f'=== Elapsed-time subjects ({t_sub_num}): ' + ' '.join(t_sub_labels) + '\n' +
                    f'=== Normal subjects ({nm_sub_num}): ' + ' '.join(nm_sub_labels))

    # Compute accuracy for every case
    def compute_rank_acc(feature, label, seq_type, probe_seq_list, gallery_seq_list, num_rank, verbose=False):
        acc = np.zeros([len(probe_seq_list), num_rank]) #- 1.
        for (p, probe_seq) in enumerate(probe_seq_list):
            for gallery_seq in gallery_seq_list:
                ...
                dist = cuda_dist(probe_x, gallery_x, metric)
                dist_sorted, idx = dist.sort(1)
                dist_sorted = dist_sorted.cpu().numpy()
                idx = idx.cpu().numpy()

                if verbose:
                    m = f'For probe: {probe_seq}:\n'
                    idx_comp = np.concatenate((np.reshape(probe_y, [-1, 1]), gallery_y[idx[:, 0:num_rank]]), axis=1)
                    m += '| Real sid ||' + ''.join(f' Pred. sid (rank {i+1}) |' for i in range(num_rank)) + '\n\n'
                    for n, row in enumerate(idx_comp):
                        m += f'|   {row[0]}   ||' + ''.join(f'   {row[i+1]} d:{dist_sorted[n, i]:03.5f}  |' for i in range(num_rank)) + '\n'
                    msg_mgr.log_info(m)
                ...
                acc[p, :] += np.sum(
                                    np.cumsum(
                                            ...
                                            1) > 0,
                                0) * 100 / dist.shape[0]
            acc[p, :] /= len(gallery_seq_list)
        acc = np.round(acc, 2)
        return acc

    # Rank accuracy for normal subjects
    nm_sub_acc = compute_rank_acc(feature, label,
                                    seq_type, probe_seq_dict[dataset],
                                    gallery_seq_dict[dataset], num_rank, verbose=True)

    # Rank accuracy for elapsed-time subjects
    t_sub_acc = compute_rank_acc(feature[t_sub_mask], label[t_sub_mask],
                                    seq_type[t_sub_mask], probe_seq_dict_T[dataset],
                                    gallery_seq_dict_T[dataset], num_rank, verbose=True)
    ...
    for i in range(num_rank):
        msg_mgr.log_info(
            '===Rank-%d (normal subjects) ===' % (i + 1))
        msg_mgr.log_info('N: %.3f,\tB: %.3f,\tS: %.3f\tMean: %.3f' % (
            nm_sub_acc[0, i],
            nm_sub_acc[1, i],
            nm_sub_acc[2, i],
            np.mean(nm_sub_acc[:, i])))

    result_dict["scalar/test_accuracy/N"] = nm_sub_acc[0, 0]
    result_dict["scalar/test_accuracy/B"] = nm_sub_acc[1, 0]
    result_dict["scalar/test_accuracy/S"] = nm_sub_acc[2, 0]

    for i in range(num_rank):
        msg_mgr.log_info(
            '===Rank-%d (elapsed-time subjects) ===' % (i + 1))
        msg_mgr.log_info('TN: %.3f,\tTB: %.3f,\tTS: %.3f\tMean: %.3f' % (
            t_sub_acc[0, i],
            t_sub_acc[1, i],
            t_sub_acc[2, i],
            np.mean(t_sub_acc[:, i])))

    result_dict["scalar/test_accuracy/TN"] = t_sub_acc[0, 0]
    result_dict["scalar/test_accuracy/TB"] = t_sub_acc[1, 0]
    result_dict["scalar/test_accuracy/TS"] = t_sub_acc[2, 0]

    # Display global weighted mean
    gl_acc = (nm_sub_num / total_sub_num) * nm_sub_acc + (t_sub_num / total_sub_num) * t_sub_acc
    gl_acc = gl_acc.mean(axis=0)

    for i in range(num_rank):
        msg_mgr.log_info(
            '===Rank-%d - Mean: %.3f ===' % (i + 1, gl_acc[i]))

    result_dict["scalar/test_accuracy/Mean"] = gl_acc[0]
